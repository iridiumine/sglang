...
class Scheduler(
    ...
    def run_batch(
        ...
        dbg_is_decode = batch.forward_mode.is_decode()
        if dbg_is_decode:
            try:
                ...
                dbg_rank = -1
            logger.info(
                "[DBG-DECODE-START] rank=%d iter=%d ts=%.3f",
                dbg_rank,
                self.forward_ct,
                batch.launch_ts,
            )
        ...
        if dbg_is_decode:
            logger.info(
                "[DBG-DECODE-END] rank=%d iter=%d dur_ms=%.1f",
                dbg_rank,
                self.forward_ct,
                (time.monotonic() - batch.launch_ts) * 1e3,
            )
