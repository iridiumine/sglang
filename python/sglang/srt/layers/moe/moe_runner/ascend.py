...
class AscendRunnerCore(MoeRunnerCore):
    ...
    def __init__(self, config: MoeRunnerConfig):
        ...
        if _uses_fused_gmm1(kernel):
            # Fused methods (MXFP8 always; MXFP4 W4A8 when
            # SGLANG_NPU_MXFP4_FUSED_SWIGLU=1) fold gate/up + swiglu + requant
            # into gmm1, so there is no separate activation step — run() skips
            # it. Left None on purpose so that reaching for it fails loudly
            # instead of silently applying an unfused swiglu to
            # already-requantised activations. This holds for both dispatchers:
            # ascend_tp gets its activation quant fused into routing, DeepEP
            # dispatches bf16 and gmm1 quantises it itself.
